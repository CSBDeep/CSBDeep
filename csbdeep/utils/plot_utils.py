...
def _plot_some(arr, title_list=None, pmin=0, pmax=100, cmap='magma', **imshow_kwargs):
	...
	def max_project(a):
		...
		proj_axis = tuple(range(1, a[0].ndim - ndim_allowed + 1))
		return np.amax(a, axis=proj_axis)
...
def to_color(arr, pmin=1, pmax=99.8, gamma=1., colors=((0, 1, 0), (1, 0, 1), (0, 1, 1))):
    ...
    arr_norm = (arr - mi) / (ma - mi + eps)

    out = np.zeros(arr_norm.shape[1:] + (3,), dtype=np.float32)
    for i, col_stack in enumerate(colors[:arr_norm.shape[0]]):
